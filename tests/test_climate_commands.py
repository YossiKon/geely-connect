...
# ------------------ rapid presets fire exactly one command, on purpose ------
# v1.21.5 followed the compound bundle with per-seat RCE_2 calls; a log on #19
# showed the bundle already carries paa.heat.11 and paa.heat.19, and that the
# follow-ups raced it - rejected with "the last request has not yet been
# executed", leaving the climate itself not running. Reverted, and pinned here
# so it does not come back.

def test_a_rapid_preset_sends_exactly_one_command():
    _ha()
    c = load("climate")
    for preset in (c.PRESET_RAPID_WARMING, c.PRESET_RAPID_COOLING):
        e, api, _ = _entity(_status())
        with _quiet_refresh(c):
            asyncio.run(e.async_set_preset_mode(preset))
        assert len(api.calls) == 1, api.calls
        assert api.calls[0][0] == "rapid"


def test_the_bundle_still_asks_for_the_seats_itself():
    """The seats travel inside the one request - which the #19 log confirms
    the server accepts as paa.heat.11 / paa.heat.19."""
    _ha()
    c = load("climate")
    e, api, _ = _entity(_status())
    with _quiet_refresh(c):
