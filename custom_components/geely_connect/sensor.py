...
def _dc_leg(data: dict) -> tuple[float, float, bool] | None:
    """The elected leg, but only while a real DC (fast-charge) session is
    delivering power - None the rest of the time.
    ...
    One resolver for both DC entities, for the same reason `_charge_leg` is one
    for all three charge sensors: split across them, the current could report a
    session the voltage called idle.
    """
    leg = _charge_leg(data or {})
    return leg if leg is not None and not leg[2] else None
...
class GeelyDCChargeCurrentSensor(CoordinatorEntity, _AutoPrecision):
    ...
    def native_value(self):
        ...
        leg = _dc_leg(self.coordinator.data or {})
        return None if leg is None else round(leg[1], 1)
